...
<div class="bg-white rounded-lg shadow-md overflow-x-auto">
  <div class="flex flex-nowrap px-6 py-4 border-b border-gray-200 font-semibold text-gray-600 bg-gray-50">
    <div class="min-w-[200px] md:min-w-0 md:w-5/12 lg:w-4/12 flex-grow-0 md:flex-grow">Estudiante</div>
    
    <div class="min-w-[200px] md:min-w-0 md:w-5/12 lg:w-4/12 flex-grow-0 md:flex-grow">Email</div>
    
    <div class="min-w-[150px] md:min-w-0 md:w-auto lg:w-2/12 flex-shrink-0">Rol</div>
    
    <div class="min-w-[120px] md:min-w-0 md:w-auto lg:w-2/12 flex-shrink-0">Acciones</div>
  </div>
  ...
  >
    <div class="min-w-[200px] md:min-w-0 md:w-5/12 lg:w-4/12 flex-grow-0 md:flex-grow flex items-center space-x-3">
    ...
    <div class="min-w-[200px] md:min-w-0 md:w-5/12 lg:w-4/12 flex-grow-0 md:flex-grow text-gray-600 truncate">{{ user.email }}</div>
    
    <div class="min-w-[150px] md:min-w-0 md:w-auto lg:w-2/12 flex-shrink-0 text-gray-600 truncate">
      {{ user.role ? 'Psicologo' : 'NA' }} 
    </div>
    
    <div class="min-w-[120px] md:min-w-0 md:w-auto lg:w-2/12 flex-shrink-0 flex space-x-2">
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900 bg-opacity-75 backdrop-blur-sm transition-opacity duration-300" 
     v-if="showAddModal || showEditModal">
  
  <div class="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 sm:p-8 transform transition-all duration-300 scale-100 opacity-100">
    
    <div class="flex justify-between items-center mb-6 border-b pb-3">
      <h2 class="text-2xl font-extrabold text-indigo-700 tracking-tight">
        {{ showEditModal ? '✍️ Editar' : '➕ Agregar' }} Psicólogo
      </h2>
      <button 
        class="text-gray-400 hover:text-indigo-600 transition-colors duration-200 p-1 rounded-full hover:bg-indigo-50" 
        @click="closeModal"
    ...
    <form @submit.prevent="saveUser" class="space-y-4">
      ...
      <div class="pt-4 flex items-center justify-end gap-3">
        ...
        <button 
          type="button" 
          class="px-5 py-2.5 rounded-xl border border-gray-300 text-gray-700 bg-white hover:bg-gray-50 transition duration-150 font-medium" 
          @click="closeModal"
        >
          Cancelar
        </button>
        ...
        <button 
          type="submit" 
          class="px-5 py-2.5 rounded-xl bg-indigo-600 text-white hover:bg-indigo-700 transition duration-150 font-semibold shadow-md hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
        >

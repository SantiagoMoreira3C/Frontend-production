        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="relative -mt-8 sm:mt-12 pb-6">
            ...
            <div class="flex flex-col items-center sm:flex-row sm:items-end sm:space-x-6">
              ...
              <div class="mt-3 mb-9 sm:mt-0 flex-1 min-w-0 text-center sm:text-left">
...
export default {
  ...
  methods: {
    ...
    // 1. FUNCIÓN PARA CARGAR LA INFORMACIÓN DEL USUARIO
    async fetchUser() {
      try {
    ...
    // Función de estudiantes (solo actualizando el dominio de la API)
    async obtenerEstudiantesActivos() {
      try {
        const token = localStorage.getItem('x-token');
        const respuesta = await axios.get(`${API_URL}api/students/?limit=1000&skip=0`, {
          headers: {
            'x-token': token,
          },
        });
  ...
  mounted() {
    this.fetchUser();
    this.obtenerEstudiantesActivos();
  }
};
</script>